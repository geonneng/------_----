...
export const joseonScenarios: Record<string, Scenario> = {
  'joseon-act1': {
    ...
    content: [
      { text: '밤은 깊었으나, 잠을 이룰 수 없었다. 개경의 사택, 내 방 창호지 너머로 차가운 바람 소리만 스산했다. 그 바람 소리를 뚫고, 땀으로 젖은 하인이 엎어질 듯 뛰어 들어왔다. 그의 눈에는 공포와 흥분이 동시에 서려 있었다.' },
      ...
      { text: '\'올 것이 왔구나.\'\n벼락이 머리를 내리치는 듯했다. 빗속에서 괴로워하던 아버님의 얼굴이 떠올랐다. 무리한 요동 정벌. 끝내 결심하셨다.\n\'반역자\'가 되시거나, \'왕\'이 되시거나.\n역사의 수레바퀴가... 지금, 거대하게 구르기 시작했다.', isThought: true }
    ],
    choices: [
  ...
  'joseon-act2': {
    ...
    content: [
      ...
      { speaker: '전령', text: '"도련님! 최영 장군이 결사항전 중입니다! 아버님께서는... 차마 왕이 계신 궁을 공격하라 명하지 못하고 망설이고 계십니다!"' },
  ...
  'joseon-act3': {
    ...
    content: [
      { text: '결국 최영 장군은 유배되었고, 우리는 개경을 장악했다. 하지만 아버지는 고뇌에 빠졌다. 우왕을 폐위시켰으나, 그 아들인 어린 창왕을 왕위에 올렸기 때문이다.' },
      { speaker: '정도전', text: '(나를 따로 불러내어) "도련님, 이래서는 안 됩니다. 우왕과 창왕은 진짜 왕씨가 아닌, 신돈의 자식들입니다! \'가짜 왕\'을 몰아내고 \'진짜 왕씨\'를 세워야 대업의 명분이 섭니다!"' },
      { text: '\'...가짜 왕이라. 참으로 무서운 말이다. 하지만... 맞는 말이다. 저들을 그대로 두면, 언젠가 우리에게 복수의 칼을 겨눌 것이다. 썩은 뿌리를 완전히 뽑아내야 한다.\'', isThought: true }
    ],
    choices: [
  ...
  'joseon-act4': {
    ...
    choices: [
      ...
      { id: 'j4-b', text: '"지금 망설이면 안 됩니다. 반대하는 늙은 신하들을 모두 파직하고, 토지 개혁을 밀어붙여야 합니다!"', label: 'B' },
  ...
  'joseon-act5-intro': {
    ...
    content: [
      { text: '1392년 봄. 아버지가 사냥 중 말에서 떨어져 사경을 헤맨다는 소식이 찬바람처럼 닥쳤다. \'이성계가 죽었다\'는 소문까지 돌았다. 기회는 이때다 싶었는지, 정몽주 선생이 칼을 뽑았다.' },
...
export const goryeoScenarios: Record<string, Scenario> = {
  'goryeo-act1': {
    ...
    content: [
      ...
      { speaker: '심복', text: '(다급하게) "대감! 포은 대감! 큰일 났습니다! 이성계 장군이... 왕의 명령을 어기고 군대를 돌렸습니다! 지금 개경을 향하고 있습니다!"' },
      { text: '\'쿵.\' 붓이 손에서 떨어져 흰 종이 위로 검은 먹물이 번졌다.\n\'이럴 수가... 북쪽 오랑캐를 막아내던 그 충신이... 결국 장군이 선을 넘었구나.\'\n\'500년 나라가... 나의 고려가 위험에 빠졌다.\'', isThought: true }
    ],
      choices: [
  ...
  'goryeo-act3': {
    ...
    content: [
      { text: '결국 최영 장군은 쫓겨났고, 이성계는 조정을 장악했다. 그는 우왕을 폐위시키고, 그 아들인 어린 창왕을 왕위에 올렸다. 나는... 패배했다. 하지만 아직 포기할 수 없었다.' },
...
export const miniGameData: Record<string, MiniGameData> = {
  'joseon-minigame': {
    ...
    faction: 'joseon',
    title: '긴급 귀환',
    description: '3번의 선택 동안 \'시간 지연\'을 피하고, \'정몽주의 추격\'을 따돌려 무사히 개경에 도착하라.',
    stages: [
      {
        stageNumber: 1,
        thought: '드디어 아버님을 찾았다. 하지만 의식이 흐릿하시다. 시간이 없다!\n\n\'아버님을 간신히 말에 태웠다. 어떻게 이동해야 하는가?\'',
        choices: [
          { id: 'j5-s1-a', text: '큰길로 달린다! (속도는 빠르나, 정몽주가 보낸 군사들과 마주칠 수 있다)', label: 'A' },
          { id: 'j5-s1-b', text: '산길로 숨어든다! (속도는 느리나, 안전하다)', label: 'B' }
        ]
      },
      {
        stageNumber: 2,
        thought: '어떻게 이 위기를 돌파할 것인가?',
        choices: [
          { id: 'j5-s2-a', text: '정면으로 돌파한다! 저들은 우리가 소수일 줄 모른다!', label: 'A' },
          { id: 'j5-s2-b', text: '여기서 밤을 새우고, 아버님을 잠시 쉬게 한다.', label: 'B' }
        ]
      },
      {
        stageNumber: 3,
      ...
      choices: [
          { id: 'j5-s3-a', text: '"문을 열라! 이성계 장군이시다!" (정체를 밝힌다)', label: 'A' },
          { id: 'j5-s3-b', text: '"밤중에 죄송하나, 급한 환자가 있소!" (정체를 숨긴다)', label: 'B' }
        ]
      }
    ],
  ...
  'goryeo-minigame': {
    ...
    faction: 'goryeo',
    title: '3일의 숙청',
    description: '3일(3번의 턴) 안에 이성계파의 핵심 인물들을 모두 제거하여 고려를 수호하라.',
    stages: [
      {
        stageNumber: 1,
        thought: 'DAY 1\n\n\'가장 먼저 누구를 쳐야 하는가? 저들의 \'머리\'인가, \'돈줄\'인가?\'',
        choices: [
          { id: 'g5-s1-a', text: '정도전, 남은 등 \'혁명의 설계자\'들을 탄핵한다!', label: 'A' },
          { id: 'g5-s1-b', text: '조준, 윤소종 등 \'토지 개혁 실무자\'들을 탄핵한다!', label: 'B' }
        ]
      },
      {
        stageNumber: 2,
        thought: 'DAY 2\n\n\'이제 저들의 \'힘\'을 무력화시킬 차례다.\'',
        choices: [
          { id: 'g5-s2-a', text: '이방원, 이방과 등 \'사병을 거느린 아들들\'을 체포한다!', label: 'A' },
          { id: 'g5-s2-b', text: '궁궐 수비대를 \'친위 쿠데타\'로 장악하여 개경을 봉쇄한다!', label: 'B' }
        ]
      },
      {
        stageNumber: 3,
        thought: 'DAY 3\n\n"대감! 이방원이 아비를 업고 성문 앞에 도착했습니다!"\n\n\'...결국 그 호랑이가 돌아왔구나. 마지막 결단이다.\'',
        choices: [
          { id: 'g5-s3-a', text: '성문을 닫고, 이방원을 역적으로 규정하여 토벌군을 보낸다!', label: 'A' },
          { id: 'g5-s3-b', text: '성문을 열어주는 척하며, 이방원만 따로 유인하여 암살한다.', label: 'B' },
          { id: 'g5-s3-c', text: '병문안을 핑계로, 이성계와 이방원을 함께 만나 회유하는 척 시간을 번다.', label: 'C' }
        ]
      }
    ],
